...
var dbpApiKey = process.env.DBP_API_KEY;
var promises = [];
votd.forEach(function(v) {
  var reference = v;
  var language = 'en';
    ...
    new rsvp.Promise(function(resolve, reject) {
      request.get({ url: dbtUrl, json: true }, function(e, r, verses) {
        if (e) {
          ...
          var formatted = formatScripture(verses, language);
...
rsvp.all(promises).then(function(verses) {
  var contents = '';
  verses.forEach(function(v) {
    contents += v.citation + '\n' + v.text + '\n\n';
  });
  fs.writeFileSync('bible-verses.txt', contents);
});
